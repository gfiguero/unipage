...
use Uni\AdminBundle\Form\DocumentType;
...
class DocumentController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if($sort) $entities = $em->getRepository('UniAdminBundle:Document')->findBy(array(), array($sort => $direction));
        else $entities = $em->getRepository('UniAdminBundle:Document')->findAll();
        $paginator = $this->get('knp_paginator');
        $pagination = $paginator->paginate($entities, $request->query->getInt('page', 1), 10);

        return $this->render('UniAdminBundle:Document:index.html.twig', array(
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $em->persist($entity);
            $em->flush();
            $request->getSession()->getFlashBag()->add( 'success', 'Document has been created.' );    
            return $this->redirect($this->generateUrl('document'));
        }

        return $this->render('UniAdminBundle:Document:new.html.twig', array(
            'entity' => $entity,
    ...
    public function showAction($id)
    {
        ...
        $entity = $em->getRepository('UniAdminBundle:Document')->find($id);

        if (!$entity) {
            throw $this->createNotFoundException('The Document cannot be found.');
        }

        $deleteForm = $this->createDeleteForm($id);

        return $this->render('UniAdminBundle:Document:show.html.twig', array(
            'entity'      => $entity,
    ...
    public function editAction($id)
    {
        ...
        return $this->render('UniAdminBundle:Document:edit.html.twig', array(
            'entity'      => $entity,
    ...
    */
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($editForm->isValid()) {
            $entity->upload();
            $em->flush();
            $request->getSession()->getFlashBag()->add( 'success', 'Document has been updated.' );
            return $this->redirect($this->generateUrl('document'));
        }
    ...
    public function deleteAction(Request $request, $id)
    {
        ...
        if ($deleteForm->isValid()) {
            ...
            $em->remove($entity);
            $em->flush();
            $request->getSession()->getFlashBag()->add( 'danger', 'Document has been deleted.' );
        }
